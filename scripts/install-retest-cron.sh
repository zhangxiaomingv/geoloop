...
#
# 特性：
#   - flock 锁防重入（/tmp/geoloop-retest.flock，常驻；与脚本内 PID 锁 .pid 分离）
#   - 绝对路径 node/npm（cron 环境 PATH 极简，nvm 装的必须全路径）
#   - 日志追加到 data/retest.log（gitignored）
#   - 幂等：重复安装只替换 geoloop-retest 那一行，其它 cron 不动
#   - 自定义节奏：RETEST_SCHEDULE="0 6 * * 1" bash scripts/install-retest-cron.sh
#
# 查看：crontab -l | grep geoloop
# 卸载：crontab -l | grep -v geoloop-retest | crontab -
...
CRON_CMD="cd $HERE && PATH=$NODE_DIR:\$PATH $FLOCK -n /tmp/geoloop-retest.flock npm run retest >> $HERE/data/retest.log 2>&1"
LINE="$SCHEDULE $CRON_CMD"

# 幂等安装：先去掉旧的 geoloop-retest 行，再加新行
TMP="$(mktemp)"
{ ( crontab -l 2>/dev/null || true ) | grep -v 'geoloop-retest' || true; } > "$TMP"
printf '%s\n' "$LINE" >> "$TMP"
crontab "$TMP"
rm -f "$TMP"
...
echo ""
echo "当前 crontab 中的 geoloop 行："
crontab -l | grep geoloop || echo "（未找到 — 安装可能未生效）"
